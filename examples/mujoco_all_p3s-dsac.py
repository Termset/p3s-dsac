...
from td3.algos import P3S_DSAC
...
def parse_args():
    ...
    args.log_dir = os.path.abspath(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), '..', 'log', env_name, 'P3S-TD3'))
...
def _init_placeholder(env):
    ...
    log_prob_a_next_ph = get_placeholder(name='log_prob_a_next', dtype=tf.float32, shape=(None, Da))
    ...
    d = {
        ...
        'beta_ph': beta_ph,
        'log_prob_a_next': log_prob_a_next_ph
    }
    return d
...
def init_actor(actor, pool, dict_ph, env, num_q, value_fn_params, noise_params):
    ...
    with tf.variable_scope(actor.name):
        ...
        for j in range(num_q):
            ...
            actor.arr_target_qf.append(
                NNQFunction(env_spec=env.spec, hidden_layer_sizes=(M1, M2), name='target_qf{i}'.format(i=j),
                            observation_ph=dict_ph['next_observations_ph'],
