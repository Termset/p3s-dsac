...
def vaule_mlp(inputs,
    ...
    with tf.variable_scope('layer0'):
        ...
        layer = nonlinearity(layer)
    # for i_layer, size in enumerate(layer_sizes[1:], 300):
    with tf.variable_scope('layer{0}'.format(1)):
        layer = affine(layer, 300,
                       W_initializer=W_initializer,
                       b_initializer=b_initializer)
        layer = nonlinearity(layer)
    with tf.variable_scope('layer{0}'.format(2)):
        layer = affine(layer, 300,
                       W_initializer=W_initializer,
                       b_initializer=b_initializer)
        layer = nonlinearity(layer)

    with tf.variable_scope('mean_layer{0}'.format(1)):
        mean_layer = affine(layer, 300,
                            W_initializer=W_initializer,
                            b_initializer=b_initializer)
        mean_layer = nonlinearity(mean_layer)
    with tf.variable_scope('mean_layer{0}'.format(2)):
        mean_layer = affine(mean_layer, layer_sizes[-1],
                            W_initializer=W_initializer,
                            b_initializer=b_initializer)
    with tf.variable_scope('std_layer{0}'.format(1)):
        std_layer = affine(layer, 300,
                           W_initializer=W_initializer,
                           b_initializer=b_initializer)
        std_layer = nonlinearity(std_layer)
    with tf.variable_scope('std_layer{0}'.format(2)):
        std_layer = affine(std_layer, layer_sizes[-1],
                           W_initializer=W_initializer,
                           b_initializer=b_initializer)

    if output_nonlinearity is not None:
        mean_layer = output_nonlinearity(mean_layer)

    if squeeze_output:
        mean_layer = tf.squeeze(mean_layer, axis=-1)

    if output_nonlinearity is not None:
        std_layer = output_nonlinearity(std_layer)

    if squeeze_output:
        std_layer = tf.squeeze(std_layer, axis=-1)

    return mean_layer, std_layer


# def out_layer(inputs,
#               layer_sizes,
#               nonlinearity=tf.nn.relu,
#               output_nonlinearity=tf.nn.tanh,
#               W_initializer=None,
...
#         layer_sizes[-1] = 1
#
#     with tf.variable_scope('layer00'):
#         layer = _bias_variable(layer_sizes[0], b_initializer)
#         for i, inp in enumerate(inputs):
#             with tf.variable_scope('input' + str(i)):
#                 layer += affine(
...
class MLPFunction(Parameterized, Serializable):
    ...
    def get_output_for(self, *inputs, reuse=False):
        with tf.variable_scope(self._name, reuse=reuse):
            value_mean, value_std = vaule_mlp(
                inputs=inputs,
                output_nonlinearity=self._output_nonlinearity,
                layer_sizes=self._layer_sizes,
            )  # N
        value_std = tf.exp(value_std)
        # value_std = value_std.exp()
        z = tf.random_normal([1])  # value_mean.get_shape()
        q_value = value_mean + tf.multiply(z, value_std)
        # tf.random_normal

        return [value_mean, value_std, q_value]
