...
class P3S_DSAC(MARLAlgorithm, Serializable):
    ...
    def __init__(
        ...
        super(P3S_DSAC, self).__init__(**base_kwargs)
    ...
    def target_q(self, r, done, q, q_std, q_next, log_prob_a_next):
        target_q = tf.stop_gradient(r + (1 - done) * self._discount * (q_next - alpha * log_prob_a_next))  # - self.alpha.detach() * log_prob_a_next)
        # if True:
    ...
    def _init_critic_update(self, actor):
        ...
        target_q_1, target_q_1_bound = self.target_q(self._dict_ph['rewards_ph'], self._dict_ph['terminals_ph'], q_1,
                                                     q_std_1,
                                                     q_next_1, self._dict_ph['log_prob_a_next'])
    ...
    def _init_training(self, env, arr_actor):
        super(P3S_DSAC, self)._init_training(env, arr_actor)
